...
case "$RESOLVED_VARIANT" in
  nvidia)
    if ! command -v nvidia-smi >/dev/null 2>&1 || ! timeout 5 nvidia-smi -L >/dev/null 2>&1; then
      echo "[h-run] WARNING: nvidia (cuda) build selected but nvidia-smi found no GPU; under v0.2.0 this build defaults to --backend cuda and HARD-ERRORS (no silent CPU fallback) — it mines NOTHING until the GPU/driver is fixed. Deliberate CPU mining is opt-in only: --backend cpu (explicit), or --backend auto --allow-cpu-fallback." | tee -a "$LOG"
    fi
    ;;
  amd)
    if ! command -v clinfo >/dev/null 2>&1 || ! timeout 5 clinfo 2>/dev/null | grep -q 'Device Type.*GPU'; then
      echo "[h-run] WARNING: amd (opencl) build selected but clinfo listed no GPU; under v0.2.0 this build defaults to --backend opencl and HARD-ERRORS (no silent CPU fallback) — it mines NOTHING until the GPU/driver is fixed. Deliberate CPU mining is opt-in only: --backend cpu (explicit), or --backend auto --allow-cpu-fallback." | tee -a "$LOG"
    fi
    ;;
esac
