...
# bounded three ways (per-probe timeout, consecutive-miss cut-off, total wall-
# clock budget) so a wedged process can never hang the hook. With 0 or 1 live
# ports we fall back to the exact single-port path this script has always used,
# so a 1-GPU rig's output is byte-identical to before.
...
# Scan bounds. Overridable by env for testing / very large rigs.
HS_MAX_GPUS="${CSD_HSTATS_MAX_GPUS:-32}"        # never probe more ports than this
...
# this file exists to fix. The port cap above and the per-probe timeout below
# already bound the scan, so this buys nothing they do not; it is kept only as an
# escape hatch for a rig that wants the scan cut short.
...
if [ "$HS_NLIVE" -eq 1 ] 2>/dev/null && \
   [ "$(printf '%s' "$HS_PORTS" | head -1)" = "$PORT" ] 2>/dev/null; then
  # Device 0 alone: scrape it verbatim => byte-identical to the pre-multi-GPU
  # single-port behaviour on a 1-GPU rig. Only when the sole survivor IS device 0;
  # a lone card 5 goes through the indexed path below so it lands at index 5.
  stats="$(hs_scrape "$PORT")"
elif [ "$HS_NLIVE" -ge 1 ] 2>/dev/null; then
  # Multi-GPU (or a lone non-zero card): one record per DEVICE INDEX, gaps padded.
...
# Nothing live, or the merge produced nothing: fall back to the original
# single-port scrape of $PORT. The subcommand prints a zero-but-valid object
# when that port is dead, so this is exactly the legacy "alive but zero" output.
[ -z "$stats" ] && stats="$(hs_scrape "$PORT")"
